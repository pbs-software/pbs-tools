...
bootBG <- function (dat="pop.pmr.qcss", K=100, S=100, R=500, SID=NULL, 
	ci=seq(0.1,0.9,0.1), lims=c("emp","norm","basic","perc","bca"), 
	allo=0, scope="G") {
	#Subfunctions--------------
	...
	mysum=function(x,f){
		...
		sum(f[z]*x[z])}
	#--------------------------
	assign("PBSfish",list(module="M04_Survey",func="bootBG"),envir=.GlobalEnv)
	require(boot)
	...
	if (!exists(snam,envir=env) || is.null(dim(dat))) {
		txt=paste("getFile(\"",snam,"\",scope=\"",scope,"\",use.pkg=TRUE,try.all.frames=TRUE); dat=",snam,sep="")
		eval(parse(text=txt)) }
	flds=names(dat)
	...
	packList(c("dat","bgtab","dStab","BStab","bgsamp","Bboot","Bqnt","Best","Bobs","group"),"PBSfish")
	invisible() }
#-------------------------------------------bootBG
...
calcMoments = function(strSpp="396", survID=c(1,2,3,121,167)) {
	...
	calcBio=function(dat,i,sa,Bonly=TRUE) {
		...
		return(Bmat) }
	#------------------------Subfunctions
	assign("PBSfish",list(module="M04_Survey",func="calcMoments"),envir=.GlobalEnv)
	getData("BOOT_DEFAULTS","GFBioSQL")
	doors=PBSdat$doorspread; names(doors)=PBSdat$SURVEY_ID
	speed=PBSdat$speed;      names(speed)=PBSdat$SURVEY_ID
	moments=list()
	for (i in survID) {
		ii=as.character(i)
		getData("gfb_survey_stratum.sql","GFBioSQL","xxx",path=.getSpath(),surveyid=i);  sa=PBSdat
		getData("gfb_survey_catch.sql","GFBioSQL",strSpp,path=.getSpath(),surveyid=i,
			doors=doors[ii],speed=speed[ii]); dat=PBSdat
		moments[[ii]]=calcBio(dat,i,sa,Bonly=FALSE)
	}
	packList(c("doors","speed","moments"),"PBSfish")
	invisible(moments)
}
#--------------------------------------calcMoments
...
getBootRuns=function(strSpp) {
	getData("gfb_boot.sql","GFBioSQL",strSpp=strSpp,path=.getSpath())
...
getPMR =  function(strSpp="396",survID=c(1,2,3,121,167)){
	assign("PBSfish",list(module="M04_Survey",func="getPMR"),envir=.GlobalEnv)
	sTime=proc.time()
	pmrTab = NULL
	...
	packList(c("pmrTab","sTime"),"PBSfish")
...
makePMRtables <- function(strSpp, surveys=list(qcss=c(1,2,3,121,167),
	...
	for (i in names(surveys)) {
		...
		for (j in sid) {
			cat(paste("SID ",j,"\n",sep="")); flush.console()
			first <- j==sid[1]
			getData(qName,"GFBioSQL",strSpp=strSpp,path=path,surveyid=j)
...
showAlpha <- function(lims=c("emp","bca")) {
	...
	packList(c("plev","xy","xcount","pobs"),"PBSfish")
	invisible() }
#----------------------------------------showAlpha
...
showIndices =  function(strSpp="396",survID=c(1,2,3,121,167), bootID){
	assign("PBSfish",list(module="M04_Survey",func="showIndices"),envir=.GlobalEnv)
	data(spn)
	surveys=getBootRuns(strSpp)
	...
	box()
	packList(c("surveys","sppBoot","survBoot","xy","cil","cih","clr","llab","ulab"),"PBSfish")
	names(y)=x
...
#-----------------------------------------------RH
simBGtrend <- function(pmr=pop.pmr.qcss,Npred=15,genT=NULL,Nsim=1,yrs=NULL,alpha=1,scope="G") {
	#Subfunctions--------------
	...
	#--------------------------
	assign("PBSfish",NULL,envir=.GlobalEnv)
	#if (scope=="L") env <- parent.frame(1) else env <- .GlobalEnv
	snam=as.character(substitute(pmr))
	txt=paste("getFile(\"",snam,"\",scope=\"",scope,"\",use.pkg=TRUE,try.all.frames=TRUE); pmr=",snam,sep="")
	eval(parse(text=txt))
	if (is.null(yrs)) {
	...
	packList(c("pmrhist","Btrend","Bmean","Bseen","brR"),"PBSfish")
...
trend <- function(strSpp="442",fqtName="gfb_iphc.sql",dbName="GFBioSQL",
	...
	assign("PBSfish",list(module="M04_Survey",func="trend",plotname="Rplot"),envir=.GlobalEnv)
...
.trend.trendy <- function() {
	...
	else eval(parse(text=paste("getFile(",fnam,",scope=\"G\",use.pkg=TRUE,try.all.frames=TRUE)",sep=""))) 
	...
	if (zstn) { # Exclude stations with no history of catch
		...
		stn <- names(PCS[PCS>0]); packList("stn","PBSfish")
	...
	Alist <- split(Y,X); packList("Alist","PBSfish")
	alist <- sapply(Alist,function(x){if(length(x[x>0&!is.na(x)])>0) x[x>0&!is.na(x)] else NULL }); packList("alist","PBSfish")
	...
	if (zero) ttlist <- sapply(Alist,PBSfish$ftran,simplify=FALSE)
	else ttlist <- sapply(alist,PBSfish$ftran,simplify=FALSE)
	...
	if (zsho) {
		...
		packList(c("n","p","xpos"),"PBSfish") }

	lmY <- lm(y[z1]~x[z1]); b <- lmY$coeff[2]; r <- PBSfish$btran(b)-one; R <- PBSfish$btran(b*(N-1))-one;
...
.trend.funky <- function() {
	...
	if (!any(tran==c("log2","log","log10"))) {
		...
		packList(c("ftran","btran"),"PBSfish")
...
.trend.booty <- function() {
	...
	brR   <- function(y,x,one) {
				...
				r <- PBSfish$btran(b)-one; R <- PBSfish$btran(b*(N-1))-one;
				return(c(b,r,R)) };
