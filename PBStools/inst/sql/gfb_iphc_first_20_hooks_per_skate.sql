...
SELECT 
  SURVEY_SERIES_ID AS surveyseriesID,
...
FROM
  ...
  WHERE
    SURVEY_SERIES_ID = 14 AND FE_MINOR_LEVEL_ID IS NOT NULL
  ) AS T
WHERE T.RN <=20
GROUP BY
  SURVEY_SERIES_ID,
...
-- getData("gfb_iphc_first_20_hooks_per_skate.sql", dbName="GFBioSQL", strSpp="435")
