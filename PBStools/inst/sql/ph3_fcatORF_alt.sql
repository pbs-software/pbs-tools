...
(SELECT
  ...
  Sum(CASE
    WHEN CS.SP_SPECIES_CDE IN ('467')  AND CS.CU_CATCH_UTLZTN_CDE NOT IN (6,22,23,24,27,28)
    THEN CS.CATSUM_ROUND_LBS_WT
...
GROUP BY
  ...
  WHERE
    CSS.\"landed\">0 OR CSS.\"discard\">0 OR CSS.POP>0 OR CSS.ORF>0 OR CSS.PAH>0 OR CSS.SBF>0 OR CSS.DOG>0 OR CSS.LIN>0 OR CSS.RFA>0
;
...
-- qu("ph3_fcatORF_alt.sql",dbName="HARVEST_V2_0",strSpp="435",server="ORAPROD",type="ORA",trusted=FALSE,uid="haighr",pwd="haighr")
