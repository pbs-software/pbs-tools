...
SELECT --TOP 20
  ...
  (CASE
    ...
    ELSE 0 END) AS GEAR,
  --TO_CHAR(OC.BEST_DATE,'YYYY-MM-DD') AS Edate,  -- SchmOracle
  CONVERT(SMALLDATETIME,CONVERT(char(10),OC.BEST_DATE, 20)) AS Edate,
  ISNULL(OC.VESSEL_REGISTRATION_NUMBER,0) AS VRN,
  (CASE
    ...
    ELSE '00' END) AS PMFC,
  --ISNULL(to_number(OC.DFO_STAT_AREA_CODE),0) AS PFMA, -- SchmOracle
  ISNULL(CONVERT(INTEGER,OC.DFO_STAT_AREA_CODE),0) AS PFMA,
  ISNULL(OC.DFO_STAT_SUBAREA_CODE,0) AS PFMS,
  --SUM(ISNULL(OC.START_LONGITUDE,1e-08)) AS S,
  --SUM(CASE WHEN OC.START_LONGITUDE IS NULL THEN 1e-08 ELSE 1 END) AS N,
  SUM(ISNULL(OC.START_LONGITUDE,1e-08)) / SUM(CASE WHEN OC.START_LONGITUDE IS NULL THEN 1e-08 ELSE 1 END) AS X,
  SUM(ISNULL(OC.START_LATITUDE,1e-08)) / SUM(CASE WHEN OC.START_LATITUDE IS NULL THEN 1e-08 ELSE 1 END) AS Y,
  SUM(ISNULL(OC.END_LONGITUDE,1e-08)) / SUM(CASE WHEN OC.END_LONGITUDE IS NULL THEN 1e-08 ELSE 1 END) AS X2,
  SUM(ISNULL(OC.END_LATITUDE,1e-08)) / SUM(CASE WHEN OC.END_LATITUDE IS NULL THEN 1e-08 ELSE 1 END) AS Y2,
  --AVG(ISNULL(OC.START_LONGITUDE,1e-08)) AS X,  -- sometimes there's a 0 record but code craps out with NULL
  --AVG(ISNULL(OC.START_LATITUDE,1e-08)) AS Y,
  --AVG(ISNULL(OC.END_LONGITUDE,1e-08)) AS X2,
  --AVG(ISNULL(OC.END_LATITUDE,1e-08)) AS Y2,
  AVG(COALESCE(OC.BEST_DEPTH_FM,OC.START_DEPTH_FM,0)) * 6. * 0.3048 AS depth,  -- convert fathoms to metres
  SUM(CASE
    WHEN OC.SPECIES_CODE IN (@sppcode) THEN ISNULL(OC.LANDED_ROUND_KG,0)
    ELSE 0 END) AS landed,
  SUM(CASE
    WHEN OC.SPECIES_CODE IN (@sppcode) THEN
      COALESCE(OC.TOTAL_RELEASED_ROUND_KG, (ISNULL(OC.SUBLEGAL_RELEASED_COUNT,0) + ISNULL(OC.LEGAL_RELEASED_COUNT,0)) * FW.MNWT, 0)
  ...
  SUM(CASE
    ...
    ELSE 0 END) AS bait,
  AVG(CASE
    WHEN OC.START_DATE IS NOT NULL AND OC.END_DATE IS NOT NULL THEN
  --TO_NUMBER(OC.END_DATE - OC.START_DATE) * 24. * 60. ELSE 0 END) AS effort,   --effort in minutes
      CONVERT(REAL, OC.END_DATE - OC.START_DATE) * 24. * 60. ELSE 0 END) AS effort,   --effort in minutes
  SUM(
...
INTO #FOSPAM
FROM GFFOS.dbo.GF_D_OFFICIAL_FE_CATCH OC LEFT OUTER JOIN
  @MEAN_WEIGHT FW ON -- FISH WEIGHTS FW
    OC.SPECIES_CODE = FW.SPECIES_CODE
WHERE
  --OC.START_LONGITUDE IS NOT NULL AND
  OC.FISHERY_SECTOR IN ('GROUNDFISH TRAWL') OR 
  (OC.FISHERY_SECTOR NOT IN ('GROUNDFISH TRAWL') AND ISNULL(OC.DATA_SOURCE_CODE,0) NOT IN (106,107))
GROUP BY
  ...
  (CASE
    ...
    WHEN OC.GEAR IN ('LONGLINE OR HOOK AND LINE','TRAP OR LONGLINE OR HOOK AND LINE') THEN 8
    ELSE 0 END),
  --TO_CHAR(OC.BEST_DATE,'YYYY-MM-DD'), -- SchmOracle
  CONVERT(SMALLDATETIME,CONVERT(char(10),OC.BEST_DATE, 20)),
  ISNULL(OC.VESSEL_REGISTRATION_NUMBER,0),
  (CASE
    WHEN OC.MAJOR_STAT_AREA_CODE IN ('01') THEN '4B'
    WHEN OC.MAJOR_STAT_AREA_CODE IN ('03') THEN '3C'
    WHEN OC.MAJOR_STAT_AREA_CODE IN ('04') THEN '3D'
    ...
    WHEN OC.MAJOR_STAT_AREA_CODE IN ('09') THEN '5E'
    ELSE '00' END),
  ---ISNULL(to_number(OC.DFO_STAT_AREA_CODE),0), -- SchmOracle
  ISNULL(CONVERT(INTEGER,OC.DFO_STAT_AREA_CODE),0),
  ISNULL(OC.DFO_STAT_SUBAREA_CODE,0)
...
SELECT --TOP 1000
  ...
  FP.VRN AS cfv,
  FP.effort AS eff, 
  CONVERT(REAL,FP.landed + FP.released + FP.liced + FP.bait) AS @sppcode
INTO #FOSMAP
FROM #FOSPAM FP
WHERE 
  FP.Edate IS NOT NULL AND
  FP.Edate>='2006' AND FP.Edate <= (SELECT CONVERT(CHAR(10),CURRENT_TIMESTAMP,23)) AND
  --FP.effort > 1 AND FP.effort <= 24*60 AND  -- can do this R
  ISNULL(FP.depth,0) <= 1600 AND
  ABS(FP.X) > 1 AND ABS(FP.Y) > 1 AND
  -FP.X + -FP.X2 < 0 AND
  FP.Y  + FP.Y2 > 0
