...
\underline{RRF}: reconstructed rockfish species (e.g., YTR)
...
\underline{gamma}: geometric mean of annual ratios of landed catch, $\sum_i \text{RRF}_i^\text{L} / \text{ORF}_i^\text{L}$, grouped by major PMFC area and fishery.
	For YTR, the reference years were set to 1998-2015 for the trawl fishery and 2007-2015 for the four non-trawl fisheries.
	
\underline{delta}: geometric mean of annual ratios of discarded catch to landed catch, $\sum_i \text{RRF}_i^\text{D} / \text{TAR}_i^\text{L}$, grouped by major PMFC area and fishery using reference years $i$= 1997-2006 for the trawl fishery and 2000-2004 for all other fisheries.
...
In two stock assessments for POP in areas 3CD and 5DE \citep{Edwards-etal:2014_pop3CD, Edwards-etal:2014_pop5DE}, the authors documented several departures from the catch reconstruction algorithm introduced by \citet{Haigh-Yamanaka:2011}.

%%++++++++++++++++++++++++++++++++++++++++++++++++
%\underline{Item~1}~~(\green{\ding{51}} RRF 2025)\\  [deprecate]
...
\bold{Item~\critem}\,:~\poptwelveone~~(\green{\ding{51}} RRF 2025) 
%\bold{Item~\critem}\,:~drop trawl and trap records from \code{PacHarv3} (1954-1995)~~(\green{\ding{51}} RRF 2025)
...
\pbsfig{CR-PH3-Problem-Areas}{Problematic statistical areas in the \code{PacHarv3} database that span multiple PMFC areas\,: 9021 (shaded green) shows DFO statistical area 102 (inshore area 2E excluded); 9270 (shaded blue) shows DFO statistical area 127 (inshore area 27 excluded).}{3.5}{}

%%++++++++++++++++++++++++++++++++++++++++++++++++
\newcommand{\poptwelvetwo}{retain use of \code{PacHarv3} data for H\&L fisheries}
\begin{itembox}
\bold{Item~\critem}\,:~\poptwelvetwo~~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\poptwelvethree~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\ytrfourteenone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\ytrfourteentwo~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\ytrfourteenthree~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\ytrfourteenfour~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\sstfifteenone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\yyrfifteenone~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\yyrfifteentwo~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\yyrfifteenthree~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\yyrfifteenfour~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\yyrfifteenfive~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\popseventeenone~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\rsreighteenone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\rsreighteentwo~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\rsreighteenthree~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\wwrnineteenone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\bornineteenone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\bornineteentwo~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\bornineteenthree~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\bornineteenfour~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\footnotesize
\centering
\caption{PacHarv3 (PH3) number of events reportedly catching each species and catch (t) of species from 1952-95 by gear type and species code,
...
\footnotesize
\raggedright
\caption{Code extract from Oracle SQL query \supquote{ph3\_fcatORF.sql} that defines catch reconstruction FIDs
...
\begin{itembox}
\bold{Item~\critem}\,:~\poptwentythreeone~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\poptwentythreetwo~~(\red{\ding{55}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\ytrtwentyfourone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
...
\begin{itembox}
\bold{Item~\critem}\,:~\sgrtwentyfiveone~~(\green{\ding{51}} RRF 2025)
\end{itembox}\\ \vspace*{1ex}
The Silvergray Rockfish (SGR) assessment \citep{Starr-Haigh:2025_sgr} added supplementary catch data, specifically for SGR (and others?) from Canadian, US, and Polish fleets \citep{Ketchen:1980_gf}, to the \supquote{rrfhistory} object.

%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\subsection{Changes adopted by \code{RRF} 2025}

A summary of the historical changes appears in Table~\ref{tab:CR_changes}, with the final column showing which changes might be adopted for an RRF assessment in 2025.
...
\footnotesize
\centering
\caption{Summary of changes made to the catch reconstruction algorithm since its inception in 2011.
Final column indicates which changes might be adopted for a typical rockfish species using a check mark (e.g., RRF 2025).
...
\hline
\tstrut Item & Assessment & Description & RRF(2025) \bstrut\\ 
\hline\tstrut
