...
FROM 
  (B01_TRIP T INNER JOIN TRIP_SURVEY TS ON 
    T.TRIP_ID=TS.TRIP_ID) INNER JOIN 
  (BOOT_HEADER BH INNER JOIN BOOT_DETAIL BD ON
    BH.BOOT_ID=BD.BOOT_ID) ON 
    TS.SURVEY_ID=BH.SURVEY_ID
WHERE 
  BD.SPECIES_CODE IN (@sppcode)
ORDER BY
  TS.SURVEY_ID, BH.BOOT_ID
