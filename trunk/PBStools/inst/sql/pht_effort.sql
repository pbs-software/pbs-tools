...
SELECT --TOP 100
  'year'  = isNull(Year(COALESCE(E.Start_FE, E.OBFL_START_DT, E.OBFL_END_DT,
    T.OBFL_DEPARTURE_DT, T.OBFL_OFFLOAD_DT)),9999),
...
WHERE 
  ...
  ISNULL(E.Duration, 0) BETWEEN 1 AND 1440 -- > 0 & <= 24 h
...
WHERE 
  ...
  (CASE WHEN C.START_DATE IS NOT NULL AND C.END_DATE IS NOT NULL THEN
    TO_NUMBER(C.END_DATE - C.START_DATE) * 24. * 60. ELSE 0 END) BETWEEN 1 AND 1440
GROUP 
  BY C.TRIP_ID, C.FISHING_EVENT_ID,
  NVL(TO_NUMBER(TO_CHAR(COALESCE(C.BEST_DATE,C.START_DATE),''YYYY'')),9999),
  TO_NUMBER(COALESCE(C.MAJOR_STAT_AREA_CODE,''00'')),
  TO_NUMBER(COALESCE(C.MINOR_STAT_AREA_CODE,''00'')),
  COALESCE(C.LOCALITY_CODE,0),
  TO_NUMBER(COALESCE(C.DFO_STAT_AREA_CODE,''00'')),
